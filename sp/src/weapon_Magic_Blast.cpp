...
//
// Purpose: This is the magic_blast weapon
//
// $Workfile:     $
// $Date:         $
// $NoKeywords: $
...
#include	"weapon_Magic_Blast.h"
#include	"grenade_Magic_Mlast.h"
...
BEGIN_DATADESC(CWeaponmagic_blast)
...
IMPLEMENT_SERVERCLASS_ST(CWeaponmagic_blast, DT_Weaponmagic_blast)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS(weapon_magic_blast, CWeaponmagic_blast);
PRECACHE_WEAPON_REGISTER(weapon_magic_blast);

acttable_t	CWeaponmagic_blast::m_acttable[] =
{
	{ ACT_RANGE_ATTACK1, ACT_RANGE_ATTACK_THROW, true },
};
IMPLEMENT_ACTTABLE(CWeaponmagic_blast);


void CWeaponmagic_blast::Precache(void)
{
	PrecacheModel("models/magicblast/models/weapons/w_magic_blast.mdl");	//<<TEMP>> need real model
	BaseClass::Precache();
}

void CWeaponmagic_blast::Spawn(void)
{
...
//------------------------------------------------------------------------------
// Purpose : Override to use magic_blasts pickup touch function
// Input   :
// Output  :
//------------------------------------------------------------------------------
void CWeaponmagic_blast::SetPickupTouch(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponmagic_blast::magic_blastTouch(CBaseEntity *pOther)
{
	// ---------------------------------------------------
	...
	// ----------------------------------------------------
	//  Give magic_blast ammo if touching client
	// ----------------------------------------------------
	if (pOther->GetFlags() & FL_CLIENT)
	{
		...
		CWeaponmagic_blast* oldWeapon = (CWeaponmagic_blast*)pBCC->Weapon_OwnsThisType(GetClassname());
		if (oldWeapon != this)
		{
			UTIL_Remove(this);
		}
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponmagic_blast::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	switch (pEvent->event)
	{
	case EVENT_WEAPON_THROW:
	{
		...
		Throwmagic_blast(launchPos, m_vecTossVelocity);
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponmagic_blast::ObjectInWay(void)
{
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponmagic_blast::WeaponLOSCondition(const Vector &ownerPos, const Vector &targetPos, bool bSetConditions)
{
...
// Output :
//-----------------------------------------------------------------------------
int CWeaponmagic_blast::WeaponRangeAttack1Condition(float flDot, float flDist)
{
...
//
//-----------------------------------------------------------------------------
void CWeaponmagic_blast::Throwmagic_blast(const Vector &vecSrc, const Vector &vecVelocity)
{
	CGrenade_magic_blast *pmagic_blast = (CGrenade_magic_blast*)Create("grenade_magic_blast", vecSrc, vec3_angle, GetOwner());

	if (!pmagic_blast)
	{
		Msg("Couldn't make magic blast!\n");
		return;
	}
	pmagic_blast->SetAbsVelocity(vecVelocity);

	// Tumble through the air
	QAngle angVel(random->RandomFloat(-100, -500), random->RandomFloat(-100, -500), random->RandomFloat(-100, -500));
	pmagic_blast->SetLocalAngularVelocity(angVel);

	pmagic_blast->SetThrower(GetOwner());
	pmagic_blast->SetOwnerEntity(((CBaseEntity*)GetOwner()));
}
...
//
//-----------------------------------------------------------------------------
void CWeaponmagic_blast::PrimaryAttack(void)
{
	...
	Throwmagic_blast(vecSrc, vecAiming * 800);
...
//
//-----------------------------------------------------------------------------
void CWeaponmagic_blast::SecondaryAttack(void)
{
	//<<TEMP>>
...
//
//-----------------------------------------------------------------------------
void CWeaponmagic_blast::DrawAmmo(void)
{
	// -------------------------------------------
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponmagic_blast::ItemPostFrame(void)
{
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponmagic_blast::CWeaponmagic_blast(void)
{
#ifdef _DEBUG
